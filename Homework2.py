...
def common_price(m, n, s, l):
    ...
    # Отличное решение! Для выполнения задачи со звездочкой - нужно использовать библиотеку Decimal. Иначе на больших числах python неверно округляет большие величины.
    # Это называется ошибкой переполнения.
...
def longest_word(sentence):
    ...
    # Отличное решение, круто, что используются регулярные выражения.
...
def uniques(repeating_string):
    ...
    for char in repeating_string:        # Пройдемся по каждому символу
        if char not in unique_chars:
            unique_chars += char
    return unique_chars
    # Решение верное, для более оптимального - см ответы.
